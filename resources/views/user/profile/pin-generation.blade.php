                                        ...
                                        <select name="package_id" class="form-control" required>
                                            ...
                                            @foreach ($packages as $package)
                                                <option value="{{ $package->id }}">
                                                    {{ $package->amount }}-{{ $package->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($pin->user)
                                                        {{ $pin->side }}
                                                    @else
                                                        <input data-id="{{ $pin['id'] }}" class="toggle-class"
                                                            type="checkbox" data-onstyle="success" data-offstyle="danger"
                                                            data-toggle="toggle" data-on="Left" data-off="Right"
                                                            {{ $pin['side'] == 'left' ? 'checked' : '' }}>
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @php
                                                        $url = $pin->pin_number;
                                                        $register = route('register', ['p' => $url]);
                                                        $message = urlencode(
                                                            "🎉 Hey 🎉\n\nI just wanted to share something awesome with you! I've been using LifeCircle and it's been a game-changer for me. I thought you might find it useful too!\n\nHere's a registration link: $register\n\nGive it a try, and we both might get some cool perks! Let me know if you have any questions about it. 😊",
                                                        );
                                                    @endphp

                                                    <button class="btn btn-primary"
                                                        onclick="navigator.clipboard.writeText('{{ $register }}')">Copy
                                                        the link</button>
